...
**Exception:** `knowledge/ganesha.md` is a Ganesha classification file, not a feature doc. It must stay Type-only (`# Ganesha` + `## Type` is `project` or `library`). Do not expand it to this structure. Do not treat it as stale for missing Overview / Design Decisions / API Surface.
...
- Task lists, progress tracking, or backlog items. If the project has a `planning/` directory, leave it alone — do not copy current tasks into `knowledge/`.
