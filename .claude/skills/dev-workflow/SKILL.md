...
Read an installed skill `<name>` from `.cursor/skills/<name>/SKILL.md` if it exists, else `.claude/skills/<name>/SKILL.md`. Companion rules live at `.cursor/rules/<name>.mdc` and `.claude/rules/<name>.md`.
...
If the issue has a `bug` label (or was created via the "Fix a bug" triage), do **not** implement ad hoc. Read the installed `fix-bug` skill (`.cursor/skills/fix-bug/SKILL.md` if it exists, else `.claude/skills/fix-bug/SKILL.md`) and follow it starting from **Phase 1** — **skip its Phase 0** (Track the Work), because the issue and branch were already created in Steps 1-2. The skill enforces the TDD discipline: understand the bug, write a failing test, make the minimal fix, and verify no regressions.
...
1. Check if the changes affect a feature area that has an existing `knowledge/*.md` file — if so, read the file and update any sections that are now stale. Rewrite sections to reflect the current state rather than appending notes. Skip `knowledge/ganesha.md` — it is a Ganesha classification file (Type-only), not a feature doc.
...
Check if a `make-a-pull-request` skill is available at `.cursor/skills/make-a-pull-request/SKILL.md` or `.claude/skills/make-a-pull-request/SKILL.md`.
...
After the PR is created, check if a code-review skill is available at `.cursor/skills/code-review/SKILL.md` or `.claude/skills/code-review/SKILL.md`.
...
2. If a code-review skill is available, re-run the code review agent (same as Step 11) to check if the fixes introduced new issues.
...
Before presenting the options, check if a code-review skill is available at `.cursor/skills/code-review/SKILL.md` or `.claude/skills/code-review/SKILL.md`.
...
1. Read the installed `code-review` skill (`.cursor/skills/code-review/SKILL.md` if it exists, else `.claude/skills/code-review/SKILL.md`).
2. Launch a `Task` subagent (`subagent_type="generalPurpose"`) with:
   - The full contents of the code-review skill as the prompt instructions
   - The PR number, owner, and repo already known from the workflow
   - A directive to skip Phase 1 (PR selection) — the PR number is already known, start from Phase 1.2
...
### 14.1 Determine the next version
...
### 14.2 Bump the version in source files
...
### 14.4 Verify the release pipeline
