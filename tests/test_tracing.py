...
OTHER_TRACEPARENT = "00-aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa-bbbbbbbbbbbbbbbb-01"

@pytest.mark.parametrize(
    ("input_data", "expected_kwargs", "expected_carrier"),
    [
        (
            {"x": 2, "y": 4, "correlation_id": W3C_TRACEPARENT},
            {"x": 2, "y": 4},
            {"correlation_id": W3C_TRACEPARENT, "traceparent": W3C_TRACEPARENT},
        ),
        (
            {"x": 2, "correlation_id": "business-id-123"},
            {"x": 2},
            {"correlation_id": "business-id-123"},
        ),
        (
        ...
        (
            {"x": 1, "correlation_id": OTHER_TRACEPARENT, "traceparent": W3C_TRACEPARENT},
            {"x": 1},
            {"correlation_id": OTHER_TRACEPARENT, "traceparent": W3C_TRACEPARENT},
        ),
        (None, {}, {}),
        ({}, {}, {}),
    ...
    ids=[
        ...
        "explicit_traceparent_tracestate",
        "explicit_traceparent_wins",
        "none_input",
        "empty_input",
        "no_reserved_keys",
    ],
...
def test_split_trace_context_does_not_mutate_input():
    ...
    input_data = {"x": 1, "correlation_id": "abc"}
    split_trace_context(input_data)
    assert input_data == {"x": 1, "correlation_id": "abc"}
...
def test_tag_current_span_sets_searchable_attributes(tracer, finished_spans):
    """correlation_id and Conductor ids are written on the current recording span."""
    with tracer.start_as_current_span("tagged"):
        tag_current_span({"correlation_id": "business-id-123"}, "conductor-task-1", "workflow-1")
    spans = finished_spans.get_finished_spans()
    assert spans[0].attributes["correlation_id"] == "business-id-123"
...
def test_tag_current_span_after_attach_writes_recording_span(tracer, finished_spans):
    ...
    activate_trace_context(
        "celery-task-tag",
        {"traceparent": carrier["traceparent"], "correlation_id": "business-id-123"},
    )
    tag_current_span(
        {"traceparent": carrier["traceparent"], "correlation_id": "business-id-123"},
        "conductor-task-1",
    ...
    assert spans[1].attributes["correlation_id"] == "business-id-123"
...
def test_conductor_task_strips_correlation_id(celery_app, celery_worker, task_poll_response, responses):
    """correlation_id in Conductor inputData is not passed to the task function."""
    worker_id = socket.gethostname()
    responses.get(
        f"https://localhost:8080/api/tasks/poll/celery_test_task?workerid={worker_id}",
        body=task_poll_response({"x": 2, "y": 4, "correlation_id": W3C_TRACEPARENT}),
    )
...
def test_conductor_task_restores_parent_trace_id(celery_app, celery_worker, task_poll_response, responses, tracer):
    """Task body sees the same trace_id as the injected correlation_id."""
    with tracer.start_as_current_span("starter") as span:
    ...
    responses.get(
        ...
        body=task_poll_response(
            {"x": 2, "y": 4, "correlation_id": carrier["traceparent"]},
...
def test_conductor_task_keeps_parent_trace_id_on_retry(celery_app, celery_worker, task_poll_response, responses, tracer):
    ...
    responses.get(
        ...
        body=task_poll_response(
            {"x": 2, "y": 0, "correlation_id": carrier["traceparent"]},
